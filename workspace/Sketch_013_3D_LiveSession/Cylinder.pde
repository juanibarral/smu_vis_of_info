class Cylinder extends AbstractGeometry
{
  ...
  public Cylinder(float _topRadius, float _bottomRadius, int _resolution, float _height)
  {
  ...
  public void init()
  {
    ...
    for(int i = 0; i < resolution; i++)
    {
      ...
      float x = bottomRadius * cos(angleStep * i);
      float z = bottomRadius * sin(angleStep * i);
      
      Vertex v0 = new Vertex(new PVector(x, 0, z));
      x = bottomRadius * cos(angleStep * (i + 1));
      z = bottomRadius * sin(angleStep * (i + 1));
      ...
      x = topRadius * cos(angleStep * i);
      z = topRadius * sin(angleStep * i);
      Vertex v3 = new Vertex(new PVector(x, -myHeight, z));
      x = topRadius * cos(angleStep * (i + 1));
      z = topRadius * sin(angleStep * (i + 1));
